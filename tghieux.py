...
def print_tghieux_banner():
    ...
    # ĐÃ SỬA DÒNG IP/MAC
    print(f" [><]{RED}{BOLD} IP:{RESET} {GLOBAL_PUBLIC_IP}")
    print(f" [><]{YELLOW}{BOLD} ID MAC:{RESET} {CYAN}{MAC_ADDRESS_HEX}{RESET}")
    print(f" [><]{YELLOW}{BOLD}{' Fb: tg.nux — Trung Hiếu(nuxw)':<63}{RESET}")
    print(f" [><]{GREEN}{BOLD}{' Zalo: 0338.316.701':<63}{RESET}")
    print(f" [><]{MAIN_FRAME_COLOR}{BOLD} Email được lưu trong {YELLOW}{BOLD}tghieu_email.txt{RESET} ")
    print(f"{CYAN}└{'─'*65}┘{RESET}")
...
def print_summary_block(email, subjects_received, scan_count):
    ...
    print(f"\n{BLUE}╔═══════════════════════════════════════════════════════════════════╗{RESET}")
    print(f" {GREEN}{BOLD}TÓM TẮT QUÉT HOÀN THÀNH:{RESET} {CYAN}{email}{RESET}")
    print(f" [><]{BLUE}{BOLD}   Lần quét tối đa:{RESET} {CYAN}{scan_count}{RESET}")
    print(f" [><]{BLUE}{BOLD}   Tổng thư đã nhận:{RESET} {CYAN}{subject_count}{RESET}")
    if subject_count > 0:
         print(f" [><]{BLUE}{BOLD}   Thư cuối cùng:{RESET} {YELLOW}{format_mail_for_display(subjects_received[-1])[:50]}...{RESET}")
    else:
         print(f" [><]{BLUE}{BOLD}   Ghi chú:{RESET} {YELLOW}Không nhận được thư nào.{RESET}")
    print(f"{BLUE}╚═══════════════════════════════════════════════════════════════════╝{RESET}")
...
def scan_noi_dung_mail_ao(email_can_kiem_tra):
    ...
    def in_khoi_quet(status_line, subject_count):
        ...
        print(f"\n{CYAN}┌{'─'*65}┐{RESET}")  
        print(f" [><]{MAIN_LABEL_COLOR}{BOLD} 📧 Email đang theo dõi:{RESET} {DETAIL_COLOR}{email_can_kiem_tra}{RESET}") 
        print(f" [><]{MAIN_LABEL_COLOR}{BOLD} 🔍 Lần quét thư:{RESET} {DETAIL_COLOR}{scan_count}{RESET} {BOLD}(1 giây/lần){RESET}") 
        print(f" [><]{MAIN_LABEL_COLOR}{BOLD} 📩 {subject_count} thư đã nhận:{RESET}") 
        ...
        if subject_count > 0 and not status_line.startswith((f"{RED}LỖI", f"{GREEN}Đã nhận Subject mới!")):
             ...
             output_line = f" [><] ✓ {GREEN}{BOLD}#{subject_count}:{RESET} {YELLOW}{BOLD}{subject_content}"
             print(f"{output_line:<65}│{RESET}") 
        else:
             print(f" {status_line:<63}{RESET}")
        
        print(f" [><]{MAIN_LABEL_COLOR}{BOLD}{YELLOW}Ctrl+C để thoát tool.{RESET}") 
        print(f"{CYAN}└{'─'*65}┘{RESET}") 
